<?php
 ...
 if ($_POST["idProfesional"] > -1){
	 $dat = $usua->getInfoProfesional($_POST["idProfesional"]);
	 $usuario = $dat[0];
	$titulo = "Modificar experiencia profesional";

 } else {
	$usuario= array("texto"=>"", "anioIni"=>"", "anioFin"=>"","orden"=>"", "idPersonal"=>$_POST["idPersonal"], "idProfesional"=>-1);
	$titulo = "Agregar experiencia profesional";

 }
?>
<form id="forma" name="forma" method="post" action="guardarPersonalPro.php" enctype="multipart/form-data">
<input type="hidden" id="idProfesional" name="idProfesional" value="<?=$_POST["idProfesional"]?>" />
